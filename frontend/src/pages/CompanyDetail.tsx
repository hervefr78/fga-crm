...
import {
  ArrowLeft, ChevronRight, Globe, Phone, Linkedin, MapPin,
  Edit2, Trash2, Copy, ExternalLink, MoreHorizontal,
  Target, Users, ListTodo, Activity as ActivityIcon, FileSearch,
  Mail, Paperclip, Smile, Sparkles, Zap, Star,
  TrendingUp, ArrowUpRight, Plus, Filter, Search,
  Video, FileText, Check, AlertCircle,
} from 'lucide-react';
...
function SideLink({ icon: Icon, label, value, href }: {
  icon: React.ElementType; label: string; value: string | null; href?: string | null;
}) {
  const content = (
    <>
...
// Composer + timeline groupee par jour. Les boutons Note/Email/Appel/RDV
// ouvrent un ComposerModal cote parent (DC8 — meme composant partage).
function ActivityFeed({
  ...
  return (
    <div>
      {/* Composer raccourci : 4 boutons qui ouvrent ComposerModal sur le bon canal */}
      <div className="m-4 border border-slate-200 rounded-lg bg-slate-50/50">
        ...
        <div className="flex items-center justify-between px-2 pb-2 pt-1 border-t border-slate-100">
          <div className="flex gap-0.5">
            ...
            <button className="inline-flex items-center gap-1 px-2 py-1 rounded text-slate-500 hover:bg-slate-100 text-xs">
              <Sparkles className="w-3 h-3" /> IA
            </button>
          </div>
          <div className="flex gap-1.5">
            <Button variant="primary" size="sm" onClick={() => onChannelClick('note')}>Publier</Button>
          </div>
        </div>
      </div>
